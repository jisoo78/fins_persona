...
export const readOrCreateEvaluationV5Retrieval = async ({
  ...
  if (!/^[a-zA-Z0-9-]+$/.test(experimentGroupId)) throw new Error('invalid Evaluation v5 group ID');
  const normalizedQuery = normalize(query);
  const queryHash = sha256(normalizedQuery);
  const filePath = path.join(evaluationV5Paths(root).retrievalCache, experimentGroupId, `${queryHash}.json`);
  try {
    const payload = JSON.parse(await readFile(filePath, 'utf8')) as {
      query: string;
      queryHash: string;
      indexHash: string;
      result: AmyHoodRetrievalResult;
      payloadHash: string;
    };
    const base = {
      query: payload.query,
    ...
    if (payload.payloadHash !== sha256(canonicalJson(base))
      ...
      throw new Error('Evaluation v5 retrieval cache is corrupt');
    }
    if (payload.indexHash !== indexHash || payload.result.trace.indexHash !== indexHash) {
      throw new Error('Evaluation v5 retrieval cache index hash is stale');
    }
    return payload.result;
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== 'ENOENT') {
      if (error instanceof SyntaxError) throw new Error('Evaluation v5 retrieval cache is corrupt');
      throw error;
    }
  }
  ...
  if (result.query !== normalizedQuery || result.trace.indexHash !== indexHash) {
    throw new Error('Evaluation v5 retrieval result is stale');
  }
