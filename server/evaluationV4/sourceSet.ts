...
import type { AdvisorSourceRecord, DecisionDomain } from '../../shared/amyHoodDecisionAdvisor';
...
export type ValidatedEvaluationV4ExternalSources = {
  sources: EvaluationV4ExternalSource[];
  events: EvaluationV4ExternalEvent[];
...
export const validateEvaluationV4ExternalSources = (
  registry: EvaluationV4ExternalSourceRegistry,
  amySources: AmySourceIdentity[],
  normalizedContentByPath: Record<string, string>,
): ValidatedEvaluationV4ExternalSources => {
  if (registry.dataset !== 'evaluation_v4_external_cfo_sources'
    || registry.version !== '4.0.0'
    || !Array.isArray(registry.sources)
    || !Array.isArray(registry.events)) {
    throw new Error('Evaluation v4 external source registry identity is invalid');
  }

  const amyUrls = new Set(amySources.map(({ canonicalUrl }) =>
  ...
  const sources = registry.sources.map((source) => {
    ...
    if (source.reviewer !== 'Codex' || !source.reviewedAt.trim()) {
  ...
  for (const event of registry.events) {
    ...
    if (secondaries.some((source) =>
      !source || source.eventId !== event.id || source.role !== 'decision_time_secondary')) {
  ...
  for (const source of sources) {
    if (!eventIds.has(source.eventId)) {
      throw new Error(`external source has unresolved event: ${source.id}`);
    }
  }
...
export const loadEvaluationV4ExternalSources = async (
  root: string,
): Promise<ValidatedEvaluationV4ExternalSources> => {
  const paths = evaluationV4Paths(root);
  const registry = JSON.parse(
    await readFile(paths.sourceRegistry, 'utf8'),
  ) as EvaluationV4ExternalSourceRegistry;
  const amyRegistry = JSON.parse(
    await readFile(advisorPaths(root).registry, 'utf8'),
  ...
  return validateEvaluationV4ExternalSources(
    registry,
    amyRegistry.sources,
