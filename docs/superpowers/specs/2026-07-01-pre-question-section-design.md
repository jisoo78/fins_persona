...
- 판단 근거 입력: 모든 문항에서 필수 저장 필드인 `rationale` 입력
- 이동 버튼: 이전, 다음, 완료
...
사용자가 선택지 없이 다음으로 이동하면 저장하지 않고 안내 메시지를 보여준다. `rationale`이 비어 있으면 다음으로 이동할 수 없다. 직접 입력 선택지를 고른 경우 직접 입력값도 비어 있으면 다음으로 이동할 수 없다.
