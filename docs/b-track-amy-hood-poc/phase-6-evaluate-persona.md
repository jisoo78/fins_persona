...
구조화 메모리의 `evaluation-context.json`에는 각 항목의 후보·사건·자료·증거 참조 목록도 함께 있어야 한다. v3는 이 참조와 텍스트에 포함된 봉인 ID·별칭을 모두 검사하고, 컨텍스트 파일 자체의 SHA-256을 실행에 고정한다. 실험 생성 이후 파일 내용이 바뀌면 실행하지 않는다. 질문 파일도 동일하게 해시를 고정한다.
...
GitHub 사건은 이전 저장소 작업에서 노출된 적이 있으므로 깨끗한 미노출 표본으로 주장하지 않고 `known_prior_exposure`로 표시한다. 봉인된 후보·사건·자료·증거 ID와 별칭은 평가 문항 작성 및 채점에서만 사용할 수 있다. 현재 구현된 Main Prompt 생성과 Evaluation v3 구조화 메모리 로딩에서는 산출물 쓰기 또는 모델 호출 전에 실패한다. 이후 정책·메모리 빌더도 `server/decisionAdvisor/leakageGuard.ts`가 제공하는 같은 공용 가드를 반드시 호출해야 한다.
