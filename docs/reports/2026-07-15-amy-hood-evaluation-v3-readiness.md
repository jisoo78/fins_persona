...
| 4대 역사적 홀드아웃 | 완료 | Main Prompt와 v3 메모리 로딩에서 후보·사건·자료·증거·별칭·원문 참조 차단 |
...
GitHub 인수는 이전 저장소 작업에서 노출된 적이 있으므로 `known_prior_exposure`로 명시했다. 깨끗한 미노출 홀드아웃으로 과장하지 않는다. 봉인 매니페스트는 실제 Main Prompt 생성과 Evaluation v3 구조화 메모리 로딩에서 해당 참조를 fail-closed 방식으로 차단한다. 실제 페르소나 생성 통합 테스트에서도 홀드아웃 source ID가 발견되면 Main Prompt 모델 호출과 파일 쓰기 전에 중단됐다. 아직 구현되지 않은 후속 정책·메모리 빌더가 같은 규칙을 재사용하도록 기존 `decisionAdvisor/leakageGuard.ts`에서 공용 가드를 다시 내보낸다.

최종 코드 리뷰에서 발견된 입력 불변성 문제도 수정했다. 실행은 포인터의 선언값만 신뢰하지 않고 실제 `evaluation-context.json` SHA-256과 공개 질문 파일 SHA-256을 고정하고 재검증한다. 구조화 메모리는 명시적 artifact reference가 없으면 거부하며, 참조 및 텍스트에 봉인 ID나 별칭이 있으면 모델 호출 전에 중단한다.
