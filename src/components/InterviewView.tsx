...
const defaultCommunicationStyle: CommunicationStyleAnswer = {
  bridge_question_id: 'communication_style',
  selected_option_id: 0,
  answer: '심층 인터뷰 단계에서 확인 필요',
};
...
const isOtherOption = (answerText: string) => {
  const optionId = parseOptionId(answerText);
  return optionId === 5 || /^E\./.test(answerText.trim()) || answerText.includes('기타');
};
...
const buildInitialMessages = (
...
): ChatMessage[] => [
  {
    ...
    text:
      phase === 'pre'
        ? `사전 질문을 시작합니다. 총 ${questions.length}개 문항을 한 번에 하나씩 진행합니다.`
        : '심층 인터뷰를 시작합니다.',
...
export const InterviewView: React.FC<InterviewViewProps> = ({
  ...
  const startDataCollection = async () => {
    ...
    try {
      ...
      setMessages(buildInitialMessages(questions, 'pre'));
      setCurrentQIndex(0);
  ...
  const savePreInterviewContext = async (context: PreInterviewContext) => {
    if (savedIntakeIds?.interviewSessionId) {
      try {
        await fetch('/api/pre-interview-contexts', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            interviewSessionId: savedIntakeIds.interviewSessionId,
        ...
        console.warn('Failed to save pre interview context', error);
      }
    }
  };
  ...
  const transitionToDeepInterview = async (
    ...
    await savePreInterviewContext(context);
    ...
    setCurrentQIndex(0);
    setMessages(buildInitialMessages(deepQuestions, 'deep'));
    setPendingPreAnswer(null);
    setInputText('');
    questionStartedAtRef.current = Date.now();
  };

  const recordPreInterviewAnswer = async (selectedAnswerText: string, customAnswerText = '') => {
    ...
    const selectedOptionId = parseOptionId(selectedAnswerText);
    const answer = customAnswerText.trim() || selectedAnswerText.replace(/^\s*\d+\.\s*/, '');
    ...
    const userMsg: ChatMessage = {
      id: `u-${Date.now()}`,
      sender: 'user',
      text: customAnswerText.trim()
        ? `${selectedAnswerText}\n직접 입력: ${customAnswerText.trim()}`
        : selectedAnswerText,
      timestamp: formatTime(),
    };
    ...
    window.setTimeout(async () => {
      ...
      const completedContext = createPreInterviewContext(nextPreAnswers, defaultCommunicationStyle);
      await savePreInterviewContext(completedContext);
      setPreInterviewAnswers(nextPreAnswers);
      setPreInterviewContext(completedContext);
      setCommunicationStyle(defaultCommunicationStyle);
      setIsComplete(true);
      setMessages((prev) => [
        ...prev,
        {
          id: `ai-pre-complete-${Date.now()}`,
          sender: 'ai',
          text: '사전 질문 40개가 완료되었습니다. 사전 질문 결과를 DB에 저장했습니다. 다음 단계는 사이드바의 심층 인터뷰에서 진행해주세요.',
          timestamp: formatTime(),
        },
      ]);
      setIsThinking(false);
  ...
  const handleSendAnswer = (answerText: string) => {
    ...
    if (interviewPhase === 'pre') {
      if (!pendingPreAnswer || !isOtherOption(pendingPreAnswer)) return;
      void recordPreInterviewAnswer(pendingPreAnswer, answerText);
      return;
    }
  ...
  const handleOptionAnswer = (answerText: string) => {
    ...
    if (interviewPhase === 'pre') {
      if (isOtherOption(answerText)) {
        setPendingPreAnswer(answerText);
        setInputText('');
        return;
      }

      void recordPreInterviewAnswer(answerText);
      setInputText('');
      return;
    }
  ...
  const runDemoFlow = async () => {
    ...
    const demoContext = createPreInterviewContext(demoPreAnswers, defaultCommunicationStyle);
    ...
    try {
      ...
      const prompt = createBrainstormerPrompt(demoProfileContext, preQuestions.length, demoSnapshot);
      let demoIntakeIds: SavedIntakeIds | null = null;

      try {
        demoIntakeIds = await saveProfileIntake(demoSnapshot, preQuestions, prompt);
        if (demoIntakeIds?.interviewSessionId) {
          await fetch('/api/pre-interview-contexts', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              interviewSessionId: demoIntakeIds.interviewSessionId,
              preInterviewContext: demoContext,
            }),
          });
        }
      } catch (error) {
      ...
      const transcript = buildInitialMessages(preQuestions, 'pre');

      preQuestions.forEach((question, index) => {
        const selectedOption = question.options?.[index % 4] ?? '1. 확인 필요';
        transcript.push({
        ...
        if (index + 1 < preQuestions.length) {
          const nextQuestion = preQuestions[index + 1];
          transcript.push({
        ...
        transcript.push({
          ...
          sender: 'ai',
          text: '데모 사전 질문 40개가 완료되었습니다. 사전 질문 결과를 DB에 저장했습니다. 다음 단계는 사이드바의 심층 인터뷰에서 진행해주세요.',
          timestamp: formatTime(),
        });
      });

      setPublicData(demoSnapshot);
      setActiveQuestions(preQuestions);
      setInterviewPhase('pre');
      setPreInterviewAnswers(demoPreAnswers);
      setCommunicationStyle(defaultCommunicationStyle);
      setPreInterviewContext(demoContext);
      setMessages(transcript);
      setCurrentQIndex(preQuestions.length - 1);
      setStep('interview');
      setIsComplete(true);
      setFinalOutput(null);
    } catch (error) {
        ...
        <div className="p-4 px-8 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800">
          <div className="flex items-center gap-3">
            <input
              ...
              disabled={isComplete || interviewPhase === 'communication' || (interviewPhase === 'pre' && (!pendingPreAnswer || !isOtherOption(pendingPreAnswer)))}
            ...
            />
            <button onClick={() => handleSendAnswer(inputText)} disabled={!inputText.trim() || isThinking || isDemoRunning || isComplete || (interviewPhase === 'pre' && (!pendingPreAnswer || !isOtherOption(pendingPreAnswer)))} className="p-3 bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-300 dark:disabled:bg-slate-800 text-white rounded-xl shadow-md transition-all flex items-center justify-center" aria-label="답변 전송">
